...
def main(opt_cut, hparams, plotter, visualizer):
    ...
    for epoch in trange(1, hparams.max_epochs + 1):
        ...
        if epoch <= hparams.epochs_only_seg_net and hparams.epochs_only_seg_net > 0:
            ...
            training_loop_seg_net(hparams, module, inner_model, hparams.batch_size_manual, train_loader_ct_labelmaps,
                                  train_losses, plotter)
            ...
            if hparams.batch_size_manual == 1:  # if batch==1, for now this is always true
                for i, batch_data_ct in tqdm(enumerate(train_loader_ct_labelmaps), total=len(train_loader_ct_labelmaps),
                    ...
                    if hparams.use_idtB:  # we know that use_idtB is always true for now
                        ...
                        cut_trainer.train_cut(module, us_sim_cut, epoch, dataloader_real_us_iterator, i,
                                              iter_data_time, epoch_iter)
        ...
        if epoch % hparams.validate_every_n_steps == 0 and epoch > hparams.epochs_only_cut:
            ...
            with torch.no_grad():
                for nr, val_batch_data_ct in tqdm(enumerate(val_loader_ct_labelmaps),
                    ...
                    # ------------------------------------------------------------------------------------------------------------------------------
                    if epoch > hparams.epochs_check_stopp_crit and hparams.plot_avg_seg_px_dff and hparams.pred_label in \
                            val_batch_data_ct[0]:
                        print(
                            f"--------------- INFER UNLABELLED REAL US IMGS FROM CUT TRAIN SET THROUGH THE SEG NET ------------")
                        # cut_model.eval()
                        try:
...
if __name__ == "__main__":
    ...
    if hparams.debug:
        ...
        hparams.exp_name += str(hparams.pred_label) + '_' + str(opt_cut.lr) + '_' + str(
            hparams.inner_model_learning_rate) + '_' + str(hparams.outer_model_learning_rate)
