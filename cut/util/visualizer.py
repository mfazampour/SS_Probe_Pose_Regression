...
def save_images_no_web(image_dir, visuals, image_path, aspect_ratio=1.0, width=256):
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result, wandb):
        print('--------------------------------------LOG IMAGES-------------------------------------------------')
        images = []
        labels = ""
        images_pil = []
        ...
        for label, image in visuals.items():
            ...
            # print('img shape: ', image_numpy.shape)
            # image_numpy = np.rot90(image_numpy, 3)
            pil_fig = torchvision.transforms.ToPILImage()(image_numpy).convert('L')
            if label_first:
                plot_fig_pil_draw = ImageDraw.Draw(pil_fig)
                font = ImageFont.truetype(fm.findfont(fm.FontProperties()), 8)
        ...
        # images_pil = [PIL.Image.fromarray(image.astype('uint8'), 'RGB') for image in images]
        wandb.log({str(labels): [wandb.Image(image) for image in images_pil]})
    ...
    def print_current_losses(self, epoch, iters, losses, t_comp, t_data, wandb):
