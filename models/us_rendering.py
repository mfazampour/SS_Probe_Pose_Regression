...
class UltrasoundRendering(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.acoustic_impedance_dict = torch.nn.parameter.Parameter(acoustic_imped_def_dict)#.to(device='cuda')
        ...
        # self.register_parameter(name='acoustic_impedance_dict', param=torch.nn.Parameter(acoustic_imped_def_dict, requires_grad=True))
        # self.register_parameter(name='attenuation_dict', param=torch.nn.Parameter(attenuation_def_dict, requires_grad=True))
        # self.register_parameter(name='mu_0_dict', param=torch.nn.Parameter(mu_0_def_dict, requires_grad=True))
        # self.register_parameter(name='mu_1_dict', param=torch.nn.Parameter(mu_1_def_dict, requires_grad=True))
        # self.register_parameter(name='sigma_0_dict', param=torch.nn.Parameter(sigma_0_def_dict, requires_grad=True))
    ...
    def map_dict_to_array(self, dictionary, arr):
        ...
        values = torch.tensor([mapping_dict[key.item()] for key in keys]).to(device='cuda')
    ...
    def rendering(self, H, W, z_vals=None, attenuation_medium_map=None, refl_map=None,
        ...
        # z = x - data_array
        # 1 - torch.sigmoid(beta * z)

        beta = 10000
        z = mu_0_map - scattering_probability
        sigmoid_map = torch.sigmoid(beta * z)
    ...
    def forward(self, ct_slice):
        ...
        boundary_map = torch.sigmoid((diff_arr != 0.).float() * 10 - 5)
        ...
        intensity_map_masked = ret_list[0] * torch.transpose(us_mask, 0, 1) #np.transpose(us_mask)
